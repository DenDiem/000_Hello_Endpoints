...
function enableButtons () {
	...
	btn = document.getElementById("input_greet_by_sname");
	btn.onclick=function(){greetByStatus();};
	
	// Update the button label now that the button is active
	btn.value="Click me for a personal status";
}
...
function greetByStatus () {
	// Get the name from the name_field element
	var sname = document.getElementById("sname_field").value;
	
	// Call the sayHelloByName() function.
	// It takes one argument "name"
	// On success, pass the response to sayHelloCallback()
	var request = gapi.client.helloworldendpoints.sayHelloByName({'name': sname});
	request.execute(sayHelloCallback);
}
